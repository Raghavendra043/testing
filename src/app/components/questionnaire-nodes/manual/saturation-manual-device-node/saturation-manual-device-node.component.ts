...
@Component({
  selector: 'app-saturation-manual-device-node',
  templateUrl: './saturation-manual-device-node.component.html',
  styleUrls: ['./saturation-manual-device-node.component.less'],
})
export class SaturationManualDeviceNodeComponent implements NodeComponent {
  ...
  getRepresentation(): Representation {
    ...
    const generateRepresentation = (
      ...
      const clickAction = (scope: QuestionnaireScope) => {
        const measurements = ['saturation', 'pulse'];

        this.parserUtils.addMeasurementsToOutput(
          scope.outputModel,
          this.nodeForm.value, // scope.nodeModel,
          node,
          measurements
        );
    ...
    const parseNode: NodeParserFunction<MeasurementNode> = (node) => {
